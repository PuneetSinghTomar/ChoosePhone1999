...
export default function laptopsComparePage() {
  ...
  useEffect(() => {
    const stored = JSON.parse(sessionStorage.getItem("selectedlaptops")) || [];
    setSelectedlaptops(stored.slice(0, 4)); // cap to 4
  }, []);
  ...
  useEffect(() => {
    if (selectedlaptops.length > 0) {
      ...
      attributes.forEach(attr => {
        ...
        const values = selectedlaptops.map(laptop => {
          const value = laptop[attr.key];
  ...
  const shouldHighlight = (attrKey, laptopIndex) => {
    return highlightedFeatures[attrKey] === laptopIndex;
  };
    ...
    <div className="w-full px-2 md:px-6 py-4">
      ...
      {selectedlaptops.length < 2 && (
        <p className="text-center text-gray-600">
          Please select at least two laptops to compare.
        </p>
      )}
      ...
      {selectedlaptops.length >= 2 && (
        <div className="hidden md:block">
          <div
            ...
            style={{
              gridTemplateColumns: `220px repeat(${selectedlaptops.length}, 1fr)`,
            }}
          >
            {/* Header Row */}
            <div className="border bg-gray-50 px-3 py-4 font-medium sticky left-0 z-10" />
            {selectedlaptops.map((ph, i) => (
              <div
                key={i}
