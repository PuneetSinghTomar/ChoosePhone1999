...
export default function smartwatchsComparePage() {
  ...
  useEffect(() => {
    const stored = JSON.parse(sessionStorage.getItem("selectedsmartwatchs")) || [];
    setSelectedsmartwatchs(stored.slice(0, 4)); // cap to 4
  }, []);

  useEffect(() => {
    if (selectedsmartwatchs.length > 0) {
      const highlights = {};
      ...
      attributes.forEach(attr => {
        ...
        const values = selectedsmartwatchs.map(smartwatch => {
          const value = smartwatch[attr.key];
  ...
  const shouldHighlight = (attrKey, smartwatchIndex) => {
    return highlightedFeatures[attrKey] === smartwatchIndex;
  };
  ...
  return (
    <div className="w-full px-2 md:px-6 py-4">
      ...
      {selectedsmartwatchs.length < 2 && (
        <p className="text-center text-gray-600">
          Please select at least two smartwatchs to compare.
        </p>
      )}
      ...
      {selectedsmartwatchs.length >= 2 && (
        <div className="hidden md:block">
          <div
            ...
            style={{
              gridTemplateColumns: `220px repeat(${selectedsmartwatchs.length}, 1fr)`,
            }}
          >
            {/* Header Row */}
            <div className="border bg-gray-50 px-3 py-4 font-medium sticky left-0 z-10" />
            {selectedsmartwatchs.map((ph, i) => (
              <div
                key={i}
            ...
            {attributes.map((attr) => (
              <React.Fragment key={attr.key}>
                ...
                {selectedsmartwatchs.map((ph, idx) => (
                  <div
                    key={idx}
