...
export default function tabletsComparePage() {
  ...
  useEffect(() => {
    const stored = JSON.parse(sessionStorage.getItem("selectedtablets")) || [];
    setSelectedtablets(stored.slice(0, 4)); // cap to 4
  }, []);
  ...
  useEffect(() => {
    if (selectedtablets.length > 0) {
      ...
      attributes.forEach(attr => {
        ...
        const values = selectedtablets.map(tablet => {
          const value = tablet[attr.key];
  ...
  const shouldHighlight = (attrKey, tabletIndex) => {
    return highlightedFeatures[attrKey] === tabletIndex;
  };
  ...
  return (
    <div className="w-full px-2 md:px-6 py-4">
      ...
      {selectedtablets.length < 2 && (
        <p className="text-center text-gray-600">
          Please select at least two tablets to compare.
        </p>
      )}
      ...
      {selectedtablets.length >= 2 && (
        <div className="hidden md:block">
          <div
            ...
            style={{
              gridTemplateColumns: `220px repeat(${selectedtablets.length}, 1fr)`,
            }}
          >
            {/* Header Row */}
            <div className="border bg-gray-50 px-3 py-4 font-medium sticky left-0 z-10" />
            {selectedtablets.map((ph, i) => (
              <div
                key={i}
