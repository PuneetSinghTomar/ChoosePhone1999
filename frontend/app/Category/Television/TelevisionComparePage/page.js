...
export default function televisionsComparePage() {
  ...
  useEffect(() => {
    const stored = JSON.parse(sessionStorage.getItem("selectedtelevisions")) || [];
    setSelectedtelevisions(stored.slice(0, 4)); // cap to 4
  }, []);

  useEffect(() => {
    if (selectedtelevisions.length > 0) {
      const highlights = {};
      ...
      attributes.forEach(attr => {
        ...
        const values = selectedtelevisions.map(television => {
          const value = television[attr.key];
  ...
  const shouldHighlight = (attrKey, televisionIndex) => {
    return highlightedFeatures[attrKey] === televisionIndex;
  };
  ...
  return (
    <div className="w-full px-2 md:px-6 py-4">
      ...
      {selectedtelevisions.length < 2 && (
        <p className="text-center text-gray-600">
          Please select at least two televisions to compare.
        </p>
      )}
      ...
      {selectedtelevisions.length >= 2 && (
        <div className="hidden md:block">
          <div
            ...
            style={{
              gridTemplateColumns: `220px repeat(${selectedtelevisions.length}, 1fr)`,
            }}
          >
            {/* Header Row */}
            <div className="border bg-gray-50 px-3 py-4 font-medium sticky left-0 z-10" />
            {selectedtelevisions.map((ph, i) => (
              <div
                key={i}
            ...
            {attributes.map((attr) => (
              <React.Fragment key={attr.key}>
                ...
                {selectedtelevisions.map((ph, idx) => (
                  <div
                    key={idx}
