...
export default function PhonesComparePage() {
  ...
  useEffect(() => {
    const stored = JSON.parse(sessionStorage.getItem("selectedPhones")) || [];
    setSelectedPhones(stored.slice(0, 4)); // cap to 4
  }, []);
  ...
  const shouldHighlight = (attrKey, phoneIndex) => {
    return highlightedFeatures[attrKey] === phoneIndex;
  };
    ...
    <div className="w-full px-2 md:px-6 py-4">
      ...
      {selectedPhones.length < 2 && (
        <p className="text-center text-gray-600">
          Please select at least two phones to compare.
        </p>
      )}
      ...
      {selectedPhones.length >= 2 && (
        <div className="hidden md:block">
          <div
            ...
            style={{
              gridTemplateColumns: `220px repeat(${selectedPhones.length}, 1fr)`,
            }}
          >
            {/* Header Row */}
            <div className="border bg-gray-50 px-3 py-4 font-medium sticky left-0 z-10" />
            {selectedPhones.map((ph, i) => (
              <div
                key={i}
